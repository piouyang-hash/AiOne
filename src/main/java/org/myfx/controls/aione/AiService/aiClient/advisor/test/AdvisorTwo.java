...
@Component
@Slf4j
public class AdvisorTwo implements BaseAdvisor {

    @Override
    public ChatClientRequest before(ChatClientRequest request, AdvisorChain chain) {
        log.info("========== [AdvisorTwo (order=2)] before 执行 ==========");
        return request;
    }

    @Override
    public ChatClientResponse after(ChatClientResponse response, AdvisorChain chain) {
        log.info("========== [AdvisorTwo (order=2)] after 执行 ==========");
        return response;
    }

    @Override
    public Flux<ChatClientResponse> adviseStream(ChatClientRequest request, StreamAdvisorChain chain) {
        log.info("========== [AdvisorTwo (order=2)] adviseStream 开始执行 ==========");
        ...
        return chain.nextStream(processedRequest)
                ...
                .doOnTerminate(() -> {
                    ...
                    after(null, null);
                    log.info("========== [AdvisorTwo (order=2)] adviseStream 结束，触发 after ==========");
                });
    }
