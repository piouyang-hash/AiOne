...
3. ThreadLocalCleanupAspect  (Order=20)
4. JwtCheckAspect            (Order=30)   【JWT鉴权，初始化User用户上下文】
5. UserIdRateLimitAspect     (Order=40)   【基于用户ID接口限流】
6. ServiceAuthAspect         (Order=50)   【服务权限校验】
7. RoleAuthAspect            (Order=60)   【角色权限校验】
8. AdminAuditAspect          (Order=100)  【最后执行：操作审计日志，优先级最低】
